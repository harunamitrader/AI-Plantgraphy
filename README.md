...
選んだ写真は送信前に長辺1600px以内のJPEGへ軽量化されます。PC側でも保存時に同じ形式へ変換するため、スマホ表示とバックアップが重くなりにくい構成です。
...
PLANT_DEX_GEMINI_ANALYSIS_MODE=full
```
...
`PLANT_DEX_GEMINI_ANALYSIS_MODE` は解析方式です。`full` は従来どおり同定と解説をまとめて生成します。`split` は先に種類だけを同定し、その後に解説文を生成します。設定ページで端末ごとに切り替えて、解析時間の「種類特定まで」を比較できます。
