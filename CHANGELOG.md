...
- 観察ヘッダーの信頼度を、正規化後の最有力候補の割合に合わせて表示（候補リスト先頭と常に一致）
- 詳細ページで写真をタップしたとき拡大表示（ライトボックス）が全画面で開かない不具合を修正
- 確認待ち（needs_review）の振り分けと図鑑解説の自動生成に使う信頼度のしきい値を、いずれも 0.65 から 0.50 に変更
