...
- basic_profile_text は必須です。項目を固定せず自由文でよいですが、一般論や写真不足の説明だけで終わらせず、その植物固有の特徴を最低1つ含めてください。120字以内です。
- visual_appeal_text は必須です。解析結果の植物について、花・葉・樹形・季節感など見た目の特徴と魅力を自然な自由文で書いてください。120字以内です。
...
JSONスキーマ:
{
  ...
  "basic_profile_text": "一般論だけでなく、その植物固有の特徴を最低1つ含めて120字以内で自由に記述",
  "visual_appeal_text": "この植物の見た目の特徴と魅力を120字以内で自由に記述",
  "care_notes": "手入れや観察メモ",
...
def normalize_result(result: dict) -> dict:
    ...
    if "basic_profile_text" not in result:
        if feature_text:
            fallback_basic = (
                f"{name}は、{feature_text}などが手がかりになる植物です。"
                "同じ種類の記録を重ねると、季節ごとの姿や生育の傾向も整理できます。"
            )
        else:
            fallback_basic = (
                f"{name}は、写真から種類を推定した植物です。"
                "花・葉・樹形などの記録を重ねると、その植物らしい特徴をより具体的に整理できます。"
            )
