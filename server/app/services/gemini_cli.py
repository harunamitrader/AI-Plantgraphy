...
- 「推定した植物名をもとに」「記録した植物」「図鑑情報として整理できます」「再解析」など、欄の説明やシステム都合の文章は禁止です。
...
def normalize_result(result: dict) -> dict:
    ...
    if isinstance(basic_text, str) and basic_text.strip() and not is_meta_text(basic_text):
        result["basic_profile_text"] = truncate_text(basic_text.strip(), 120)
    else:
        result.pop("basic_profile_text", None)

    if isinstance(visual_text, str) and visual_text.strip() and not is_meta_text(visual_text):
