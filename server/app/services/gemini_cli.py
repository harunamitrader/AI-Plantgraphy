...
- basic_profile_text は必須です。写真に写っている特徴ではなく、推定した植物名をもとに、その植物自体の図鑑的な特徴を自由文で書いてください。一般論だけで終わらせず、その植物固有の特徴を最低1つ含めてください。120字以内です。
- visual_appeal_text は必須です。写真に写っている特徴ではなく、推定した植物名をもとに、その植物自体の見た目の特徴や魅力を自然な自由文で書いてください。120字以内です。
...
JSONスキーマ:
{
  ...
  "basic_profile_text": "推定した植物名をもとに、その植物自体の図鑑的な特徴を120字以内で自由に記述",
  "visual_appeal_text": "推定した植物名をもとに、その植物自体の見た目の特徴や魅力を120字以内で自由に記述",
  "care_notes": "手入れや観察メモ",
...
def normalize_result(result: dict) -> dict:
    ...
    if "basic_profile_text" not in result:
        fallback_basic = (
            f"{name}は、推定した植物名をもとに記録した植物です。"
            "種類固有の特徴は、再解析や追加入力で図鑑情報として整理できます。"
        )
    ...
    if "visual_appeal_text" not in result:
        result["visual_appeal_text"] = truncate_text(
            f"{name}は、推定した植物名をもとに見た目の魅力を記録した植物です。"
            "花・葉・樹形や季節ごとの印象は、図鑑情報として整理できます。",
            120,
        )
