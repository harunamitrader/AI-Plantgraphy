...
- 基本的な特徴、見た目の魅力、手入れメモなどの図鑑向け解説文は書かないでください。
- uncertainty_notes は120字以内にしてください。
...
PROFILE_PROMPT = """植物名をもとに、図鑑ページに載せる解説文を作ってください。
返答は必ずJSONのみです。挨拶、説明、Markdown、コードフェンス、JSON外の文章は禁止です。
...
- care_notes は一般的な育て方・剪定・病害虫・置き場所など、実用的な手入れメモを書いてください。
- 「推定した植物名をもとに」「記録した植物」「図鑑情報として」などのメタ文は避けてください。
