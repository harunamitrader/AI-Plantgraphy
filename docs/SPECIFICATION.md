...
AI Plantgraphy は、スマホ側を「入力と閲覧」、自宅PC側を「保存、解析、Web配信」の中心に置くローカルファースト構成である。
...
7. スマホでURLまたはQRコードを開き、必要ならPWAとしてホーム画面へ追加する。
...
12. Jinja2テンプレートで図鑑、観察記録、確認待ち、設定ページをサーバー描画する。
...
| Web画面 | `server/app/web/templates`, `server/app/web/static` | HTML、CSS、PWA、アップロード画面のJavaScript |
...
現在の図鑑・観察一覧・植物詳細はJSON APIではなく、FastAPIとJinja2によるHTMLページとして提供する。

| パス | 説明 |
| --- | --- |
| `/` | ホーム |
| `/plants` | 図鑑一覧 |
| `/plants/{plant_id}` | 植物詳細 |
| `/observations` | 観察記録一覧 |
| `/observations/{observation_id}` | 観察詳細 |
| `/upload` | 植物追加 |
| `/review` | 確認待ち |
