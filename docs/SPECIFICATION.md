...
```mermaid
flowchart LR
  A["スマホWeb/PWA<br/>3枚撮影・選択"] --> B["Tailscale<br/>プライベート接続"]
...
- ユーザーは1回の観察記録につき写真を3枚撮影または選択できる。
- 写真は送信前にプレビューできる。
...
- 自宅PCがスリープ中またはPlant Dex未起動の場合は利用できないことを接続ガイドに表示する。
