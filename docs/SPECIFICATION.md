...
Plant Dex は、スマホで庭木・草花の写真を3枚撮影し、自宅PC上の Gemini CLI で植物を解析して、植物図鑑として後からスマホで見返せるようにする個人用アプリケーションである。

初期版では、スマホアプリは「撮影・送信」に集中し、図鑑表示は自宅PCが提供するWebページで行う。将来的に、同じAPIを使ってスマホアプリ内に図鑑画面を実装できる構成にする。
...
- 主な役割:
  - 写真を3枚撮影する
  - 任意メモを入力する
  - 画像を自宅PCの受信APIへアップロードする
  - 図鑑Webページをブラウザまたはアプリ内WebViewで開く
...
```mermaid
flowchart LR
  A["Androidアプリ<br/>3枚撮影"] --> B["Cloudflare Tunnel<br/>HTTPS公開"]
...
- 利用候補パッケージ:
  ...
  - `path_provider`
  - `webview_flutter`
  - `geolocator` または `location`、位置情報対応時
...
- アプリは写真3枚を1セットとしてPCサーバーへ送信する。
- 送信先はHTTPS URLとする。
- 初期版ではCloudflare TunnelのURLを設定値として持つ。
...
- 初期版では外部ブラウザまたはWebViewでWeb図鑑を開く。
- 将来的にはアプリ内ネイティブ画面で一覧・詳細を表示できるようにする。

### 7.2 PCサーバー

#### F-SRV-001 画像受信

- `/api/observations` で画像3枚を受け取る。
...
- 自宅PCのAPIはCloudflare Tunnel経由でHTTPS公開する。
- 初期版でも簡易APIキーを必須にする。
- APIキーはAndroidアプリ設定とPCサーバー環境変数に保持する。
- Discordのユーザートークンは使用しない。
- Discord連携はWebhookまたはBotトークンのみ使用する。
- 公開Web図鑑には最低限の認証を検討する。
...
- 位置情報は初期版では任意・後回しとする。
- Web図鑑を外部公開する場合は、位置情報や個人宅が特定される情報を非表示にする。
...
- AndroidアプリとPCサーバーはAPI契約で疎結合にする。
...
### 10.6 POST `/api/observations/{id}/assign-plant`
...
- ユーザーは植物名を修正できる。
- ユーザーは観察記録を別の植物へ移動できる。
- ユーザーは複数の植物レコードを統合できる。
...
- スマホまたはテストクライアントから画像3枚を送信できる
- 画像が観察記録フォルダに保存される
- Gemini CLIで画像3枚の解析が実行される
- 解析結果がSQLiteに保存される
