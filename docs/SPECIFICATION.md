...
3. FastAPIはAPIキーを検証し、画像枚数、形式、ファイルサイズを確認する。
...
| 接続診断 | `server/app/services/connectivity.py`, `diagnostics.py` | Tailscale、LAN URL、Gemini、APIキー状態の確認 |
...
| `/upload` | 植物追加 |
| `/review` | 確認待ち |
| `/settings` | 接続、診断、APIキー、Geminiモデル、場所ラベル、バックアップ |
