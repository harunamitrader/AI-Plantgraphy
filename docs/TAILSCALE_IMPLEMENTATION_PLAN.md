# AI Plantgraphy Tailscale 実装計画

## 1. 方針

AI Plantgraphy の外出先利用は Tailscale を標準ルートにする。

AI Plantgraphy は自宅PC上の Gemini CLI とローカル保存を中核にするため、クラウド公開型よりも「自分の端末だけが入れるプライベートネットワーク」が合っている。Tailscale を使うことで、スマホから自宅PCの AI Plantgraphy に安全にアクセスできる。

```text
外出先のスマホ
  ↓ Tailscale
自宅PCの AI Plantgraphy
  ↓
...
```text
1. PCにAI Plantgraphyを入れる
2. PCとスマホにTailscaleを入れる
3. 両方で同じTailscaleアカウントにログインする
4. PCでAI Plantgraphyを起動する
5. AI Plantgraphyの接続ページに出たQRコードをスマホで読む
6. 外出先でも写真を3枚送信できる
```
...
```text
%USERPROFILE%\Desktop\AI Plantgraphy を起動.lnk
```

起動対象:

```text
AI-Plantgraphy\scripts\start_plant_dex.ps1
```
