(function () {
  ...
  async function saveDraft(draft) {
    const apiBaseUrl = draft.apiBaseUrl || (window.AIPlantgraphyApp && window.AIPlantgraphyApp.getApiBaseUrl()) || '';
    const payload = {
  ...
  async function checkHealth() {
    const app = window.AIPlantgraphyApp;
    if (!app) {
      throw new Error('アプリ設定を読み込めません。');
    }
    const response = await fetch(app.apiUrl('/api/health'), { cache: 'no-store' });
    if (!response.ok) {
  ...
  async function checkHealthAt(apiBaseUrl) {
    const app = window.AIPlantgraphyApp;
    if (!app) {
      throw new Error('アプリ設定を読み込めません。');
    }
    const baseUrl = app.normalizeApiBaseUrl(apiBaseUrl);
    if (!baseUrl) {
  ...
  async function sendDraft(id, apiKey) {
    ...
    const apiBaseUrl = draft.apiBaseUrl || (app && app.getApiBaseUrl()) || '';
    if (!apiBaseUrl) {
      throw new Error('この下書きの送信先URLがありません。設定から自分のPC URLを保存し直してください。');
    }
    ...
    const response = await fetch(`${app.normalizeApiBaseUrl(apiBaseUrl)}/api/observations`, {
      method: 'POST',
      headers: {
